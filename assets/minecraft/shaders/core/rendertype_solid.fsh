...
void main() {
    ...
    if(!adjacentCheck(alpha, 242.0) || !adjacentCheck(alpha, 241.0)) {
        color *= showRedAndGray(vertexColor, FogColor, isGui) * ColorModulator;
    }
    
    color = apply_emissive_perspective_for_item(color, vec4(1.0), tintColor, vertexColor, vertexDistance, zPos, isGui, FogStart, FogEnd, alpha, screenSize, screenFragCoord, GameTime);
